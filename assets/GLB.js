...
GLB.resized = function(e, _skipLayout){
	...
	//For case and blablabloom page (scrolling sideways without seeing rubberband y scroll)
	if(GLB._iOS && GLB._hasTouch && GLB._vwOuter < 1181 && GLB._scrollAxis == 1){
...
function GLBImage(_url, _holder, _w, _h, _className, onloadCallback, _insertBefore, _alt){
	...
	/*_this.reappend = function(){
		if(!_removed) return;
		_holder.appendChild(_this.img);
	}*/
	//Prevent dragging
	function md(e){
...
function overWriteLink(_a){
	...
	var _scrollto = _a.getAttribute("data-scrollto") || "";	
	...
	function clicked(e){
		...
		if(_scrollto != "") _scrollToModule = _scrollto;
		_router.setUrl(_href);
	}
	function dragstart(e){e.preventDefault();}
	//Add listeners if not absolute path (email or tel)
	if(_href.indexOf("http") == -1 && _href.indexOf("mailto:") == -1 && _href.indexOf("tel:") == -1 && _href.indexOf("//") != 0 && _a.className.indexOf("person") == -1){
		if(_router._useAPI){
